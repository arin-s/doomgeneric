...
static void handleKeyInput(){
  ...
  while (SDL_PollEvent(&e)){
    ...
    if (e.type == SDL_KEYDOWN) {
      ...
      // DUMP FRAME TO DISK (press period)
...
void DG_DrawFrame()
{
  if(dumpheap == true) {
    // dumps frame instead
    FILE* fdlog = fopen("doomgenericLOG.txt", "w");
    fprintf(fdlog, "Starting log!\n");
    FILE* fd = fopen("FRAME.h", "w");
    if (fd == NULL) {
      perror("Error opening file");
      exit(1);
    }
    fprintf(fd, "const uint8_t framebuffer[] = {");
    for(int i = 0; i < (DOOMGENERIC_RESX * DOOMGENERIC_RESY); i++) {
      ...
      fprintf(fd, "0x%02x,", px[0]);
      fprintf(fd, "0x%02x,", px[1]);
      fprintf(fd, "0x%02x", px[2]);
      //fprintf(fd, "0x%02x,", px[3]);
      if((i != DOOMGENERIC_RESX * DOOMGENERIC_RESY - 1))
        fprintf(fd, ",");
      fprintf(fd, "\n");
    }
    fprintf(fd, "};");
